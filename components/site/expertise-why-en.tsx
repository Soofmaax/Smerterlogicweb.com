...
export function ExpertiseWhyEN() {
  const counters = [
    { label: "PageSpeed score", end: 95, unit: "/100" },
    { label: "Sites delivered", end: 20, unit: "+" },
    { label: "Response within", end: 24, unit: "h" },
  ...
  return (
    <section className="mx-auto w-full max-w-5xl px-6 py-12">
      <div className="mb-8 text-center">
        <h2 className="font-heading text-3xl font-semibold md:text-4xl">Why my expertise</h2>
        <p className="mt-2 text-foreground/70">Concrete benefits for your local business.</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <Block
          ...
          desc="Ultra-fast static sites, polished SEO and UX, clean and maintainable code."
          counter={counters[0]}
        />
        <Block
          icon={<Users className="h-5 w-5" />}
          title="Craftsmen & SMBs"
          desc="Focused on local businesses’ needs and lead generation."
          counter={counters[1]}
        />
        <Block
          icon={<Target className="h-5 w-5" />}
          title="Total transparency"
          desc="Clear pricing, code ownership, no vendor lock‑in."
          counter={counters[2]}
        />
        <Block
          icon={<Award className="h-5 w-5" />}
          title="Full support"
          desc="Training provided, responsive support, strategic guidance."
          counter={counters[3]}
        />
      </div>
    </section>
